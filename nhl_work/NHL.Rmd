...
cols_to_convert <- c("G", "PTS", "PlusMinus", "A",
                     "PIM", "EVG", "PPG", "SHG", "GWG",
...
- First we removed *Assists* as it is easier to get the number of goals from the number of assists (0-2 assists is added for each goal depending on the play beforehand)
...
- We decided to use this final model
- Although some variables are not significant in our model we believe we shouldn't leave them out as they still carry important information regarding the teams.

```{r ábrák}
# olyan ábra, amin látszik, az összes gól (db) és más színnel a test adatsor
ggplot(teach_data,aes(Goals)) + geom_histogram(bins = 50) + 
  geom_histogram(data = test_data, aes(fill = "red", alpha = 0.5),bins = 50, show.legend = FALSE) +
  theme_minimal()
